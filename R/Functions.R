...
#' in \code{newClass}.
#' @seealso \code{\link{as.numeric}}, \code{\link{as.character}}, \code{\link{as.factor}}
#'  which this function wraps
#' @details The function also checks if one is converting a factor to numeric variable and it will make a correct
...
changeClass <- function(df, newClass) {
    # should assert that df is data frame and that newClass has the same number of columns
...
# Rep for vectors
# ----------------------------------------------------------------------
#' A function that replicates a vector and creates a matrix where each vector is either a row or a column.  
#' 
#'
#' @param vec A vector of any mode.
...
#' @details A simple wrapper for \code{\link{rep}} that takes a vector and creates a matrix by replicating it certain number of times either as rows or as columns.
#'
#' @export
#' @examples
#' vec <- c(1,2,3,4)
